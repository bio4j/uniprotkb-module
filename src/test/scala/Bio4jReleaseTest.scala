...
object TestBundles {
  ...
  case object TestBundle 
    extends ReleaseBundle(UniprotKBRelease.s3address, UniprotKBRelease.module) {
      override def install[D <: AnyDistribution](d: D): InstallResults = {
        checkGorilla(new NodeRetrieverTitan(
            new Bio4jManager(UniprotKBRelease.module.dbLocation.getAbsolutePath))) -&-
        UniprotKBRelease.install(d) -&-
        UniprotKBDistribution.install(d) -&-
        checkGorilla(UniprotKBDistribution.nodeRetriever)
      }
  }
